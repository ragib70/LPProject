 ...
 */
export default function deposit(recipientAddress, daiAmount) {
  ...
  const daiAmountBN = floatToWeb3BN(daiAmount, 18, BigNumber.ROUND_DOWN);

  try {
    return all41Exchange.methods
      .depositToWalletPool(recipientAddress, daiAmountBN)
      .send();
  } catch (error) {
